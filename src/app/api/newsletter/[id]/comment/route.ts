...
export async function POST(
  ...
  await connectDB();
  const { userId, userName, userImage, text, userEmail } = await req.json();
  if (!userId || !text?.trim())
  ...
  const adminHtml = emailWrapper(`
    <tr><td style="padding:30px 30px 20px;">
      <div style="display:inline-block;background-color:#e74a4a;color:#ffffff;padding:8px 20px;border-radius:20px;font-size:12px;font-weight:600;text-transform:uppercase;letter-spacing:1px;">New Comment</div>
    </td></tr>
    <tr><td style="padding:0 30px 20px;">
      <h2 style="margin:0;color:#2a1315;font-size:22px;font-weight:600;font-family:'Playfair Display',serif;">New comment on your article</h2>
    </td></tr>
    ${postImageBlock}
    <tr><td style="padding:0 30px 20px;">
      <table role="presentation" style="width:100%;border-collapse:collapse;">
        ${infoRow("Article", post.title)}
        ${infoRow("Commented By", userName)}
        ${infoRow("Comment", text.trim())}
        ${infoRow("Total Comments", String(post.comments.length))}
      </table>
    </td></tr>
  `);

  sendMail(ADMIN_EMAIL, `New Comment on "${post.title}"`, adminHtml).catch(
    console.error,
  );
  ...
  if (!recipientEmail && userId) {
    ...
    if (user) recipientEmail = (user as { email: string }).email;
  }

  if (recipientEmail) {
    const userHtml = emailWrapper(`
      <tr><td style="padding:30px 30px 20px;">
        <div style="display:inline-block;background-color:#e74a4a;color:#ffffff;padding:8px 20px;border-radius:20px;font-size:12px;font-weight:600;text-transform:uppercase;letter-spacing:1px;">Comment Posted</div>
      </td></tr>
      <tr><td style="padding:0 30px 20px;">
        <h2 style="margin:0;color:#2a1315;font-size:22px;font-weight:600;font-family:'Playfair Display',serif;">Your comment was posted!</h2>
        <p style="margin:10px 0 0;color:#6b7280;font-size:14px;line-height:1.6;">Thank you for engaging with our newsletter, ${userName}.</p>
      </td></tr>
      ${postImageBlock}
      <tr><td style="padding:0 30px 20px;">
        <table role="presentation" style="width:100%;border-collapse:collapse;">
          ${infoRow("Article", post.title)}
          ${infoRow("Your Comment", text.trim())}
        </table>
      </td></tr>
      <tr><td style="padding:0 30px 30px;">
        <p style="margin:0;color:#6b7280;font-size:13px;line-height:1.6;">You'll receive a notification when someone replies to or likes your comment.</p>
      </td></tr>
    `);

    sendMail(recipientEmail, `Your comment on "${post.title}"`, userHtml).catch(
      console.error,
    );
  }
