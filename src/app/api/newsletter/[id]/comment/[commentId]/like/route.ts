      ...
      if (authorEmail) {
        const userHtml = emailWrapper(`
          <tr><td style="padding:30px 30px 20px;">
            <div style="display:inline-block;background-color:#e74a4a;color:#ffffff;padding:8px 20px;border-radius:20px;font-size:12px;font-weight:600;text-transform:uppercase;letter-spacing:1px;">Comment Liked</div>
          </td></tr>
          <tr><td style="padding:0 30px 20px;">
            <h2 style="margin:0;color:#2a1315;font-size:22px;font-weight:600;font-family:'Playfair Display',serif;">Someone liked your comment!</h2>
            <p style="margin:10px 0 0;color:#6b7280;font-size:14px;line-height:1.6;">Your comment is getting attention.</p>
          </td></tr>
          ${postImageBlock}
          <tr><td style="padding:0 30px 20px;">
            <table role="presentation" style="width:100%;border-collapse:collapse;">
              ...
              ${infoRow("Total Likes", String(comment.likes.length))}
            </table>
          </td></tr>
        `);

        sendMail(authorEmail, `${userName} liked your comment`, userHtml).catch(
          console.error,
        );
      }
    }
